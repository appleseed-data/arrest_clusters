from src.utilities.make_dataprep import parse_cols
...
def prep_crosswalk(filename, sheet_name='CPD_crosswalk_final'):
    ...
    micro_charge_map = df[['description', Config.micro_col]].copy(deep=True)
    micro_charge_map = micro_charge_map.drop_duplicates()

    macro_charge_map = df[[Config.micro_col, Config.macro_col]].copy(deep=True)
    macro_charge_map = macro_charge_map.drop_duplicates()

    police_related_map = df[['description', Config.police_col]].copy(deep=True)
...
def apply_crosswalk_directmatch(df, micro_charge_map=None, macro_charge_map=None, police_related_map=None):
    ...
    if micro_charge_map is not None:
        ...
        micro_charge_map = micro_charge_map.to_dict()[Config.micro_col]
    ...
    if police_related_map is not None:
        ...
        police_related_map = police_related_map.to_dict()[Config.police_col]

    # map any charges that match crosswalk exactly
    n_cols = len(Config.charge_columns)

    for idx in range(n_cols):
        df[Config.charge_columns_micro[idx]] = df[Config.charge_columns[idx]].map(micro_charge_map)
        df[Config.charge_columns_macro[idx]] = df[Config.charge_columns_micro[idx]].map(macro_charge_map)
        df[Config.police_related_flags[idx]] = df[Config.charge_columns[idx]].map(police_related_map)
...
def apply_crosswalk_fuzzymatch(df, micro_charge_map, macro_charge_map, police_related_map, max_edit_distance=4):
    ...
    micro_charge_dict = micro_charge_dict.to_dict()[Config.micro_col]
    # return a dictionary version of the cpd crosswalk
    macro_charge_map = macro_charge_map.set_index(Config.micro_col)
    macro_charge_map = macro_charge_map.to_dict()[Config.macro_col]
    # return a dictionary version of the police related crosswalk
    police_related_dict = police_related_map.set_index('description')
    police_related_dict = police_related_dict.to_dict()[Config.police_col]
